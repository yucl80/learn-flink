   ...
   val data = stream.map(new MapFunction[String, AccLog] {
      override def map(value: String): AccLog = {
        try {
          ...
          val timestamp = new Timestamp(sdf.parse(t).getTime)
    ...
    data.keyBy("system").window(TumblingEventTimeWindows.of(Time.seconds(5)))
...
case class AccLog(system: String, sessionid: String, clientip: String, status: Double, bytes: Double, time: Double,timestamp: Timestamp,count :Int)